        ...
        *,
            ...
            try:
                ...
                if attempt >= self.max_retries:
                    raise TikHubUpstreamError(
                        "TikHub request failed after retries: "
                        f"{_transport_error_message(exc, method, self.base_url, path)}"
                    ) from exc
                await asyncio.sleep(self.retry_backoff * (attempt + 1))
        ...
        if isinstance(last_error, (httpx.TimeoutException, httpx.TransportError)):
            raise TikHubUpstreamError(
                ...
                f"{_transport_error_message(last_error, method, self.base_url, path)}"
            )
...
def _transport_error_message(
    ...
    return (
        f"{type(exc).__name__} while requesting {method.upper()} {base_url}{path}: "
        f"{error_text}. Check network, proxy, DNS/TUN routing, and TikHub reachability."
    )
