...
class ResearchScheduler:
    ...
    async def schedule_job(
        ...
        *,
        ...
        job = await self._refresh_scheduled_job_window(job)
    ...
    async def _refresh_scheduled_job_window(self, job: dict[str, Any]) -> dict[str, Any]:
        if not job.get("schedule_enabled"):
            return job
        comment_policy = job.get("comment_policy") or {}
        if comment_policy.get("time_start") and comment_policy.get("time_end"):
            return job
        start_date, end_date = _rolling_job_dates(job, comment_policy)
        if start_date == job.get("start_date") and end_date == job.get("end_date"):
            return job
...
def _rolling_job_dates(job: dict[str, Any], comment_policy: dict[str, Any]) -> tuple[date, date]:
    if comment_policy.get("disable_time_window"):
        return date(1970, 1, 1), date.today()
    preset = str(comment_policy.get("time_preset") or "").lower()
    preset_days = {
        "1d": 1,
        "7d": 7,
        "30d": 30,
    ...
    if preset_days:
        end_date = date.today()
        start_date = end_date - timedelta(days=preset_days - 1)
