...
class ResearchKeywordSet(Base):
    ...
    __table_args__ = (
        UniqueConstraint("org_id", "name", name="uq_research_keyword_set_name"),
    )
...
class ResearchCreatorProfile(Base):
    ...
    __table_args__ = (
        UniqueConstraint("org_id", "platform", "creator_id", name="uq_research_creator_profile"),
    )
...
class ResearchCreatorSearchSession(Base):
    __tablename__ = "research_creator_search_sessions"
    ...
    view_state_json = Column(json_column(), nullable=False, default=dict)
    diagnostics_json = Column(json_column(), nullable=False, default=dict)
    realtime_json = Column(json_column(), nullable=False, default=dict)
    progress_json = Column(json_column(), nullable=False, default=dict)
    message = Column(Text, nullable=True)
    result_summary = Column(Text, nullable=True)
    result_count = Column(Integer, nullable=False, default=0)
    saved = Column(Boolean, nullable=False, default=False, index=True)
...
class ResearchCreatorSearchSessionResult(Base):
    __tablename__ = "research_creator_search_session_results"
    __table_args__ = (
        UniqueConstraint(
            "session_id",
            "rank",
            name="uq_research_creator_search_session_result_rank",
        ),
    )
    ...
    session_id = Column(
        Integer,
        ForeignKey("research_creator_search_sessions.id"),
        nullable=False,
        index=True,
    )
    ...
    match_score = Column(Float, nullable=True)
    snapshot_json = Column(json_column(), nullable=False, default=dict)
    created_at = Column(DateTime(timezone=True), nullable=False, server_default=func.now())
...
class ResearchGrowthProject(Base):
    ...
    __table_args__ = (
        UniqueConstraint("org_id", "name", name="uq_research_growth_project_name"),
    )
...
class ResearchLead(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "project_id",
            "external_lead_id",
            name="uq_research_lead_external",
        ),
    )
    ...
    project_id = Column(
        Integer,
        ForeignKey("research_growth_projects.id"),
        nullable=False,
        index=True,
    )
...
class ResearchLeadAttributionResult(Base):
    ...
    lead_id = Column(Integer, ForeignKey("research_leads.id"), nullable=False, index=True)
    conversion_event_id = Column(
        Integer,
        ForeignKey("research_lead_conversion_events.id"),
        nullable=False,
        index=True,
    )
...
class ResearchLeadAttributionSpend(Base):
    __tablename__ = "research_lead_attribution_spend"
...
class ResearchMonitorPool(Base):
    ...
    __table_args__ = (
        UniqueConstraint("org_id", "name", name="uq_research_monitor_pool_name"),
    )
...
class ResearchContentSample(Base):
    ...
    __table_args__ = (
        UniqueConstraint("org_id", "platform", "content_id", name="uq_research_content_sample"),
    )
...
class ResearchContentTracker(Base):
    ...
    schedule_interval_minutes = Column(Integer, nullable=False, default=720)
    enabled = Column(Boolean, nullable=False, default=True, index=True)
    latest_analysis_run_id = Column(Integer, nullable=True, index=True)
    latest_analysis_snapshot_id = Column(Integer, nullable=True, index=True)
    created_at = Column(DateTime(timezone=True), nullable=False, server_default=func.now())
    updated_at = Column(
        DateTime(timezone=True),
        nullable=False,
...
class ResearchContentTrackerAnalysisRun(Base):
    __tablename__ = "research_content_tracker_analysis_runs"

    id = Column(Integer, primary_key=True)
    org_id = Column(Integer, nullable=True, index=True)
    tracker_id = Column(Integer, ForeignKey("research_content_trackers.id"), nullable=False, index=True)
    status = Column(String(32), nullable=False, default="completed", index=True)
    analysis_version = Column(String(32), nullable=False, default="v1", index=True)
    window_days = Column(Integer, nullable=False, default=7)
    ...
    trend_strength_score = Column(Float, nullable=False, default=0.0)
    noise_rate = Column(Float, nullable=False, default=0.0)
...
class ResearchContentTrackerAnalysisSnapshot(Base):
    __tablename__ = "research_content_tracker_analysis_snapshots"
    ...
    run_id = Column(
        Integer,
        ForeignKey("research_content_tracker_analysis_runs.id"),
        nullable=False,
        index=True,
    )
    snapshot_date = Column(Date, nullable=False, index=True)
    status = Column(String(32), nullable=False, default="ready", index=True)
    overview_json = Column(json_column(), nullable=False, default=dict)
    trends_json = Column(json_column(), nullable=False, default=dict)
    keywords_json = Column(json_column(), nullable=False, default=dict)
    patterns_json = Column(json_column(), nullable=False, default=dict)
    creators_json = Column(json_column(), nullable=False, default=dict)
    samples_json = Column(json_column(), nullable=False, default=dict)
    risks_json = Column(json_column(), nullable=False, default=dict)
    decisions_json = Column(json_column(), nullable=False, default=dict)
    meta_json = Column(json_column(), nullable=False, default=dict)
    created_at = Column(DateTime(timezone=True), nullable=False, server_default=func.now(), index=True)


class ResearchContentTrackerCandidateSample(Base):
    __tablename__ = "research_content_tracker_candidate_samples"
    __table_args__ = (
        UniqueConstraint(
            "run_id",
            "platform",
            "platform_post_id",
            name="uq_research_content_tracker_candidate_sample",
        ),
    )
    ...
    url = Column(Text, nullable=True)
    publish_time = Column(DateTime(timezone=True), nullable=True, index=True)
    candidate_level = Column(String(8), nullable=False, default="L2", index=True)
    similarity_score = Column(Float, nullable=False, default=0.0)
    ...
    fingerprint_json = Column(json_column(), nullable=False, default=dict)
    engagement_json = Column(json_column(), nullable=False, default=dict)
    evidence_json = Column(json_column(), nullable=False, default=dict)
    created_at = Column(DateTime(timezone=True), nullable=False, server_default=func.now(), index=True)
...
class ResearchCollectionRun(Base):
    __tablename__ = "research_collection_runs"
    ...
    mode = Column(String(32), nullable=False, default="collect_only", index=True)
    trigger_source = Column(String(32), nullable=False, default="manual", index=True)
    status = Column(String(32), nullable=False, default="queued", index=True)
    phase = Column(String(32), nullable=False, default="queued", index=True)
    job_id = Column(Integer, ForeignKey("research_jobs.id"), nullable=True, index=True)
    ...
    started_at = Column(DateTime(timezone=True), nullable=True)
    completed_at = Column(DateTime(timezone=True), nullable=True)
    request_payload_json = Column(json_column(), nullable=False, default=dict)
    summary_json = Column(json_column(), nullable=False, default=dict)
    error_json = Column(json_column(), nullable=False, default=dict)
    created_at = Column(DateTime(timezone=True), nullable=False, server_default=func.now(), index=True)
    updated_at = Column(
        DateTime(timezone=True),
        nullable=False,
...
class ResearchCompetitorAccount(Base):
    ...
    __table_args__ = (
        UniqueConstraint("org_id", "platform", "creator_id", name="uq_research_competitor_account"),
    )
...
class ResearchAccountProfile(Base):
    ...
    __table_args__ = (
        UniqueConstraint("org_id", "platform", "account_id", name="uq_research_account_profile"),
    )
