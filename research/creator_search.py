...
def aggregate_creator_profile(
    *,
    ...
    return {
        ...
        "avg_engagement_rate": _avg_engagement_rate(posts, base_profile.get("follower_count")),
        "hot_post_rate": _hot_post_rate(posts),
    ...
    *,
    ...
    for (creator_platform, current_creator_id), creator_posts in posts_by_creator.items():
        ...
        profile = aggregate_creator_profile(
            ...
            base_profile=_base_profile_from_posts(current_creator_id, creator_posts),
        )
        profile["tag_summary_json"]["analysis_version"] = analysis_version
...
def _avg_engagement_rate(posts: list[dict[str, Any]], follower_count: int | None) -> float | None:
    if not posts or not follower_count:
        return None
    total = sum(_engagement_total(post) for post in posts)
    return round(total / max(1, follower_count * len(posts)), 6)


def _hot_post_rate(posts: list[dict[str, Any]]) -> float | None:
    if not posts:
        return None
    totals = [_engagement_total(post) for post in posts]
    if not totals:
        return 0.0
