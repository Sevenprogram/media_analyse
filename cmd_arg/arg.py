...
async def parse_cmd(argv: Optional[Sequence[str]] = None):
    ...
    def main(
        ...
        max_notes_count: Annotated[
            ...
            typer.Option(
                "--max_notes_count",
                help="Maximum number of posts/videos/notes to crawl for this run",
                rich_help_panel="Basic Configuration",
            ),
        ...
        sort_mode: Annotated[
            ...
            typer.Option(
                "--sort_mode",
                help="Normalized search sort mode: relevance/latest/most_liked/most_commented/most_collected",
                rich_help_panel="Basic Configuration",
            ),
        ] = getattr(config, "CRAWLER_SORT_MODE", "relevance"),
        time_preset: Annotated[
            str,
            typer.Option(
                "--time_preset",
                help="Normalized search time preset: all/1d/7d/30d/180d",
                rich_help_panel="Basic Configuration",
            ),
        ] = getattr(config, "CRAWLER_TIME_PRESET", "all"),
        time_start: Annotated[
            Optional[str],
            typer.Option(
                "--time_start",
                help="Exact requested search range start as ISO datetime",
                rich_help_panel="Basic Configuration",
            ),
        ] = getattr(config, "CRAWLER_TIME_START", None),
        time_end: Annotated[
            Optional[str],
            typer.Option(
                "--time_end",
                help="Exact requested search range end as ISO datetime",
                rich_help_panel="Basic Configuration",
            ),
        ] = getattr(config, "CRAWLER_TIME_END", None),
        max_results_per_keyword_per_platform: Annotated[
            Optional[int],
            typer.Option(
                "--max_results_per_keyword_per_platform",
                help="Maximum saved results for each keyword on each platform",
                rich_help_panel="Basic Configuration",
            ),
        ...
        fill_strategy: Annotated[
            ...
            typer.Option(
                "--fill_strategy",
                help="Result fill strategy for exact time ranges",
                rich_help_panel="Basic Configuration",
            ),
        ...
        max_extra_pages: Annotated[
            ...
            typer.Option(
                "--max_extra_pages",
                help="Extra pagination budget for prefer-fill exact range searches",
                rich_help_panel="Basic Configuration",
            ),
        ...
        config.CRAWLER_SORT_MODE = sort_mode
        config.CRAWLER_TIME_PRESET = time_preset
        config.CRAWLER_TIME_START = time_start
        config.CRAWLER_TIME_END = time_end
        ...
        return SimpleNamespace(
            ...
            sort_mode=config.CRAWLER_SORT_MODE,
            time_preset=config.CRAWLER_TIME_PRESET,
            time_start=config.CRAWLER_TIME_START,
            time_end=config.CRAWLER_TIME_END,
            max_results_per_keyword_per_platform=config.CRAWLER_MAX_RESULTS_PER_KEYWORD_PER_PLATFORM,
            fill_strategy=config.CRAWLER_FILL_STRATEGY,
            max_extra_pages=config.CRAWLER_MAX_EXTRA_PAGES,
        )
