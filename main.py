...
class CrawlerFactory:
    ...
    def create_crawler(platform: str) -> AbstractCrawler:
        if getattr(config, "ENABLE_TIKHUB", False):
            if platform not in CrawlerFactory.SUPPORTED_PLATFORMS:
                supported = ", ".join(sorted(CrawlerFactory.SUPPORTED_PLATFORMS))
        ...
        crawler_class = CrawlerFactory._load_legacy_crawler(platform)
    ...
    @staticmethod
    def _load_legacy_crawler(platform: str):
        try:
            if platform == "xhs":
            ...
            raise RuntimeError(
                "Legacy Playwright crawler modules are not included in this API-only build. "
                "Set ENABLE_TIKHUB=true to use the TikHub API crawler."
            ) from exc
        return None
